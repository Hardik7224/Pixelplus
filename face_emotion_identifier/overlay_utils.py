...
NEON_BLUE  = (0, 255, 255)    # cyan glow
NEON_PINK  = (255, 0, 255)    # magenta accent
NEON_YELLOW= (255, 255, 0)    # yellow accent
NEON_WHITE = (245, 245, 245)  # base text
...
def draw_fps(img, fps: float, pos=(10, 24), color=NEON_PINK):

    txt = f"FPS: {int(fps):02d}"
    cv2.putText(img, txt, pos, cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2, cv2.LINE_AA)
...
def draw_glitch_text(
    ...
    text: str,
    pos=(20, 40),
    base_color=(0, 255, 255),  # NEON_BLUE
):
    x, y = pos
    # shadow layer 
    cv2.putText(
...
def draw_status_panel(
    ...
    lines: List[str],
    pos=(10, 60),
    bg_color=(20, 20, 30),  # dark background so neon text is readable
    alpha: float = 0.6,
):
    ...
    img[:] = cv2.addWeighted(img, 1.0 - alpha, overlay, alpha, 0.0)
